...
class CertificateController extends Controller
{
        ...
        try{
            ...
            if (!empty($old_certificate)){
              ...
              if(!is_null($request->certificate_attach_file)){
                ...
                $this_year = date('Y');
                $old_certificates = Certificate::where('year',$this_year)->get();
                $total_certificates = count($old_certificates);

                $certificate = new Certificate();
                $certificate->application_id = $id;
                $certificate->certificate_no = $this_year.'.'.($total_certificates+1);
            ...
            $application->save();
            DB::commit();
            return redirect()->route('workHandover/application/forward', ['id' => $id])->with('success_msg', 'আবেদন অনিস্পত্তি তৈরি সফলভাবে সম্পূর্ণ হয়েছে');
        }catch (\Exception $ex){
            DB::rollBack();
    ...
    public function view($id, $userId){
        try {
            if(Gate::check('handoverIsInTask', ['admin:cm:cv:cs:cd', $userId]))
            {
                $application = Application::findOrFail($id);

                return view('handover.certificate.view', [
    ...
    public function issue(Request $request, $id, $userId)
    {
        try {
            if(Gate::check('handoverIsInTask', ['admin:cv', $userId]))
            {
                $certificate = Certificate::where('application_id', $id)->first();
                if (!empty($certificate)){
                    $certificate->is_issue = 1;
                    $certificate->issue_by = $userId;
                    $certificate->on_behalf_of = Auth::id();
                    $certificate->issue_ip = $request->ip();
                    $certificate->issue_date = date('y-m-d');
                    $certificate->save();
                    return redirect()->route('workHandover/application/forward', ['id' => $id])->with('success_msg', 'আবেদন অনিস্পত্তি ইস্যু সফলভাবে সম্পূর্ণ হয়েছে');
                }
            }
            return view('404');
        }
    ...
    public function complete($id, $userId)
    {
        DB::beginTransaction();
        try {
            if(Gate::check('handoverIsInTask', ['admin:cd', $userId]))
            {
                $application = Application::findOrFail($id);
                if (!empty($application))
                {
                    ...
                    $v_message->user_id = $userId;
                    $v_message->on_behalf_of = Auth::id();
                    ...
                    DB::commit();
                    return redirect()->route('workHandover/Application')->with('success_msg', 'আবেদন সম্পুর্ন হয়েছে');
                }
           }
           return view('404');
        }
