...
@section('title', 'চিঠি ইস্যু নিবন্ধন রেজিস্টার')
@section('content')
    <p class="m-0 text-black-50">চিঠি ইস্যু নিবন্ধন রেজিস্টার</p>
    <hr>
    <!-- Top Statistics -->
    <div class="card">
        <div class="card-header">
          <div class="row">
              ...
              <div class="col-md-4 text-right">
                 <form action="{{ route('report/letterIssue') }}">
                ...
                <table class="table table-sm table-bordered table-hover">
                    ...
                    @foreach($letters as $i => $letter)
                        <tr>
                            ...
                            <td>
                                <ul>
                                    <li>{{ $letter->application->user->name }}, <strong>পিতাঃ  </strong>{{ $letter->application->personalInfo->fname }}</li>
                                    @foreach($letter->application->personalInfo->applicants as $applicant)
                                        <li>{{ $applicant->applicant_name }}, <strong>পিতাঃ  </strong>{{ $applicant->father_name }}</li>
                                    @endforeach
                                </ul>
                            </td>
                            <td class="text-center align-middle">
                                {{ $letter->application->personalInfo->pa_house_no }},
                                {{ $letter->application->personalInfo->pa_road_no }},
                                {{ $letter->application->personalInfo->pa_sector_no }},
                                {{ $letter->application->personalInfo->pa_area }},
                                {{ $letter->application->personalInfo->pa_post }},
                                {{ $letter->application->personalInfo->pa_thana }},
                                {{ $letter->application->personalInfo->pa_district }},
                            </td>
                            <td class="text-center align-middle">{{ \App\Http\Helpers\Helper::ConvertToBangla($letter->application->app_id) }}</td>
                        </tr>
                    @endforeach
