<div class="card">
    <div class="card-header text-right">
        @if($application->is_certificate_make == 1)
            <a href="{{ route('certificate/reset', ['id' => encrypt($application->id), 'type' => $type]) }}" class="btn btn-sm btn-danger btn-flat confirm-alert"><i class="fas fa-redo-alt"></i> রিসেট</a>
        @endif
    </div>
    <div class="card-body">
        <form action="{{ route('certificate', ['id' => encrypt($application->id), 'type' => $type]) }}" method="post" autocomplete="off" enctype="multipart/form-data">
            @csrf
